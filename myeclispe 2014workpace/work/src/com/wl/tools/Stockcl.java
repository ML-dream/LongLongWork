...
public class Stockcl {
	...
	public static void Stockin(HttpServletRequest request,String item_id,String item_name,String spec,String item_type,String warehouse_id,String stock_id,double num1,double unitPrice,String unit){
		HttpSession session = request.getSession();
	    String createPerson = ((User)session.getAttribute("user")).getStaffCode();
	    SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");//设置日期格式
	    String createTime = df.format(new Date());
	    String monitorSql="insert into stockmonitor values(?,?,to_date(?,'yyyy-mm-dd,hh24:mi:ss'),?,?)";
	    String[] params={item_id,num1+"",createTime,createPerson,"P"};
		...
		try {
			...
			for(int i=0,l=stockList.size();i<l;i++){
				...
				if(item_id.equals(stock.getItemId())){
					...
					Sqlhelper.executeUpdate(monitorSql, params);
					b=true;
				}
	...
	public static void Stockout(HttpServletRequest request,String item_id,double num1){
		HttpSession session = request.getSession();
	    String createPerson = ((User)session.getAttribute("user")).getStaffCode();
	    SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");//设置日期格式
	    String createTime = df.format(new Date());
	    String monitorSql="insert into stockmonitor values(?,?,to_date(?,'yyyy-mm-dd,hh24:mi:ss'),?,?)";
	    String[] params={item_id,num1+"",createTime,createPerson,"M"};
		String sql="select ITEM_ID,STOCK_NUM from stock where item_id='"+item_id+"'";
		List<StockInfo> stockList=new ArrayList<StockInfo>();
		try {
