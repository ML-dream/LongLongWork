...
public class EditEmployeeInfoServlet extends HttpServlet {
	...
	throws ServletException, IOException {
	    ...
	    EmployeeSql = 
	    	...
	    	"from (select A.*,ROWNUM row_num from (select EM.* from employee_info EM where leave like '%"+leave+"%'and em.section_code like '%"+section+"' and em.work_type like '%"+workType+"'   order by STAFF_CODE asc) A where ROWNUM<="+(countPerPage*pageNo)+" and leave like '%"+leave+"%' order by STAFF_CODE) B " +
