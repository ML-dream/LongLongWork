...
public class seeEditSupplierDataMaintenanceServlet extends HttpServlet {

	/**
	 * Constructor of the object.
	 */
	public seeEditSupplierDataMaintenanceServlet() {
		super();
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String totalCountSql="select count(*) from editsupplierpaystatistics";
		try {
		...
		String editSql="select B.companyid,maintenancedate,editendpayment,initialendpayment,editperson,edittime,reason,C.companyname,D.staff_name editPersonName from " +
				"(select A.*,rownum rn from(select EM.* from editsupplierpaystatistics EM order by companyid asc,maintenancedate desc) A where rownum<="+pageSize*pageNow+" " +
