...
/*
 ...
 /*   function waixiecheck(){
	    ...
	    data.orderId=mini.get("orderId").getValue();
	    data.productId=mini.get("productId").getValue();
	    data.issueNum=mini.get("issueNum").getValue();
	    var params = eval("("+mini.encode(data)+")");
	    var url='WaiXieCheckServlet';
    	jQuery.post(url, params, function(data){
    		data = mini.decode(data);
        	mini.get("isCo").setValue(data[0].isWaiXie);
 	   		},'json');
    }
    */
	...
	function ondEdit(foNo,foId){
	    ...
	    window.parent.document.getElementById("subIframe").src="BaseServlet?meth=FodetailData&pathTo=fodetailData&productType="+productType+"&productId="+productId+"&issueNum="+issueNum+"&foNo="+foNo+"&foId="+foId;
	}
