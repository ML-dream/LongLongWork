...
.mini-rgantt .myitem2 .mini-gantt-percentcomplete {
	...
	<body>
		...
		<label for="dragUpdown">
			允许上下拖拽
		</label>
		<input type="checkbox" id="linkInFactory" />
		<label for="linkInFactory">
			允许设备内联动
		</label>

		<input type="button" onclick="load()" value="刷   新" />
		<input type="button" onclick="save()" value="保  存" />


		<div id="viewCt" style="margin-top: 8px;height: 400px;width: 1000px;">
		</div>
        ...
        <table style="font-size:13px;">
        	<tr>
        		<td>工序号：</td>
        		<td><input id="taskID" class="mini-textbox" allowInput="false"></td>
        	</tr>
            <tr>
                <td>工序名称：</td>
...
gantt.setColumns([
    { header: '<span style="font-weight:bold;font-size:13px;color:#44474a;">设备名称</span>', 
        field: "Name",
         width: 180,
          name: "taskname",
           cellCls: "cellbg",
...
gantt.on('itemtooltipneeded', function (e) {
    ...
    e.tooltip = "<div>任务：" + task.Name + "</div>"
                    ...
                    +   "<div style='float:right;'>工期："+task.Duration/3600 + "小时</div></div>"
...
function ShowTaskWindow() {
    ...
    if (task) {
        ...
        taskWindow.show();
		mini.get("taskID").setValue(task.UID);
...
function ok() {
    ...
    node = gantt.getOwnerNode(selectedTask);
    var taskWindow = mini.get("taskWindow");
    taskWindow.hide();

    var machineId = node.UID;
    ...
    var taskStart=mini.get("taskStart").getFormValue("yyyy-MM-dd HH:mm:ss");
    var taskFinish=mini.get("taskFinish").getFormValue("yyyy-MM-dd HH:mm:ss");
    /*
	...
	*/
    $.ajax({
        //url: "../JSP/testReceive.jsp",
        //url: "/test/wlaction.do?flag=save",
        //url: "AddGT?taskID="+taskID+"&taskName="+taskName+"&percentComplete="+percentComplete+"&taskStart="+taskStart+"&taskFinish="+taskFinish,
        //encodeURI(encodeURI(taskStart))
        url: "AddGT?taskID="+taskID+"&taskName="+encodeURI(encodeURI(taskName))+"&percentComplete="
        	+percentComplete+"&taskStart="+encodeURI(encodeURI(taskStart))+"&taskFinish="
        	+encodeURI(encodeURI(taskFinish))+"&itemId="+itemId+"&machineId="+machineId,
        type: "POST",
        success: function (text) {
            alert("保存成功");
        }
...
function split() {
    ...
    if (task) {
        ...
        if (task.Duration <= 1 * 24 ) {
            alert("不能拆分小于等于1小时的任务");
            return;
        }
