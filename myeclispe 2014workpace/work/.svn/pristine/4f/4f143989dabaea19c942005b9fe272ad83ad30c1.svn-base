		...
		工艺
	   	...
	   	<table style="text-align: right;border-collapse:collapse;" border="gray 1px solid;"  width="100%">
	   		...
	   		<tr>
            	...
            	<td><input id="productTypeName" name="productTypeName" class="mini-textbox"  width="100%"  value="${result.productTypeName}" readonly/>
	   		...
	   		<tr>
            	...
            	<td><input id="matirial"  name="matirial" class="mini-combObox"  allowInput="true" width="100%" url= "LoadStuff"  value="${result.matirial}" text="${result.matirial}"/></td>
            ...
            var url = 'BaseServlet?meth=AddFoHeader1&pathTo=';
 		    jQuery.post(url, params, function(data){
 		    	if(data.result =="success"){
 		    		//alert("操作成功，点击左侧树刷新页面 ");
 		    		parent.refresh();		//调用父页面的刷新函数
 		    		window.location.href =
 		    			 "BaseServlet?meth=GetFoHeader&pathTo=foHeader&orderId="+para.orderId+"&productId="+para.productId+"&FProductId="+para.fproductId+"&isHere=1";
 		    		//Gofodetail();
 		    		//parent.doB("listfo");
 		    	}else{
