...
public class showAddDataMaintenanceServlet extends HttpServlet {

	/**
	 * Constructor of the object.
	 */
	public showAddDataMaintenanceServlet() {
		super();
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String viewSql="create or replace view PayStatisticsServletView as select companyid,min(maintenancedate) maintenanceDate from paystatistics group by companyid";
		...
		String dateSql="select companyid,maintenanceDate from (select A.*,rownum rn from (select EM.* from PayStatisticsServletView EM order by companyid asc) A " +
	...
	// TODO: handle exception
		...
		for(int i=0,l=maintenancedateList.size();i<l;i++){
			...
			String Sql="select maintenancedate,A.companyid,endpayment,B.companyname from paystatistics A " +
