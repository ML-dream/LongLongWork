...
//		String staffcode = ((User)session.getAttribute("user")).getStaffCode();
	    ...
	    String OrderSql= "select B.ORDER_ID,PRODUCT_ID,ISSUE_NUM,B.LOT,SORTIE," +
	    		...
	    		"PRODUCT_NAME,B.SPEC,MADEPLACE,drawingId,B.unitprice," +
	    		"planProfit,customerkoukuan,shijixiaoshouer,yijinghuikuan,yukuan," +
	    		"isduizhanghan,islailiao,isjiaohuo,D.customer,D.connector,C.companyName,E.deptname,B.issue_num,B.quotationtotal,F.Stock_num stocknum " +
	    	...
	    	"left join Stock F on B.Product_Id=F.Item_id "+
	    	" order by "+orderStr;
