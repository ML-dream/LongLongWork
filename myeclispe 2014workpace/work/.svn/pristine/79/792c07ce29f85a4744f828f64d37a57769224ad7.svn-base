  ...
  <body style="background-color: #EBEFF5;">
        ...
        <div class="mini-splitter" style="width:100%;height:100%;" borderStyle="border:0;">
            ...
            <div showCollapseButton="false" style="border:0px;" >
                ...
                >        
                    <div title="当日任务监测" url="<%=basePath %>chejianjiance/jianceFirst.jsp" >        
                    </div>
                </div>
            </div>        
        </div>
        ...
        function showTab(node) {
			...
			if(node.pid == "-1"){				//根节点
				if (!tab) {
					...
					tab.url = "<%=basePath%>GojianceServlet?flag=gen&orderId="+ node.id;
					tabs.addTab(tab);
				}
			}else{
				if (!tab) {
					...
					tab.title = node.pid + node.id;
					tab.showCloseButton = true;
					tab.url = "<%=basePath%>GojianceServlet?flag=ye&orderId="+ node.pid+"&itemId="+node.id;
