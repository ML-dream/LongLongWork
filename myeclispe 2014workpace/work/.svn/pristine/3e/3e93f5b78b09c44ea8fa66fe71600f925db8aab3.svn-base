...
<html>
  ...
  <body style="height:100%;">
    ...
    <td><input id="companyId" name="companyId" class="mini-buttonedit" width="100%" onbuttonclick="onButtonEdit" textName="companyName" required="true" 
                allowInput="false"/></td>
    <td><label for="endPayment$text">初期应回款</label></td>
    ...
    <div id="grid" class="mini-datagrid" style="width:100%;height:85%;"
         ...
         url="showAddDataMaintenanceServlet" >
    ...
    function ondEdit(row_uid){
    	...
    	if(row){
            ...
            $.ajax({
			     url: "getAddDataMaintenanceFormServlet?companyId="+row.companyId,
    ...
    function updateRow(){
    	...
    	if(editform.isValid()==false){
    		...
    		$.ajax({
    			type:"post",
    			url:"editDataMaintenanceServlet?initialEndPayment="+initialEndPayment,
    			data:{submitData:json},
    			dataType:"json",
    			success:function(data){
    				alert(data.result);
    ...
    function editReport(){
    	 window.location="payMent/seeEditDataMaintenanceServlet.jsp";
    }
