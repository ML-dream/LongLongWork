  ...
  <body>
  	...
  	<fieldset id="allDetail" style="width: 100%;" align="center">
		...
		<td><input id="warehouse_id" name="warehouse_id" class="mini-buttonedit" width="100%" textName="warehouse_name" onbuttonclick="onButtonEditWarehouse" 
  		allowInput="false" required="true" enabled="false" value="${checkin.warehouseId }" text="${checkin.warehouseName }"/></td>	
        ...
        enabled="false" allowInput="false" value="${checkin.companyId }" text="${checkin.companyName}"/>
   		</td>
   		<td><label for="deptid$text">部门</label></td>
        <td><input id="deptid" name="deptid" class="mini-buttonedit" width="100%" textName="deptname" onbuttonclick="onButtonEditDept" value="${checkin.deptId }" text="${checkin.deptName }"
  		allowInput="false" /></td>
   		<td><label for="checkin_kind$text">入库类型</label></td>
        <td><input id="checkin_kind"  name="checkin_kind" class="mini-textbox"  width="100%" required="true" enabled="false" value="${checkin.checkinKind}"/></td>
        </tr>
        ...
        <td><label for="delivery$text">交货人</label></td>
         ...
         allowInput="false" required="true" enabled="false" value="${checkin.delivery }" text="${checkin.deliveryName }"/>  
         </td>
         ...
         allowInput="false" required="true" enabled="false" value="${checkin.examine }" text="${checkin.examineName }"/> </td>
         ...
         allowInput="false" required="true" enabled="false" value="${checkin.admin }" text="${checkin.adminName }"/>  
         </td>
   		</tr>
   	 </table>
   ...
   function onButtonEditDept(e){
	  ...
	  //  function onOperatePower(e) {
	   //     var str = "";
	  //     str += "<span>";
	  //      str = "<a style='margin-right:2px;' title='编辑' href=javascript:ondEdit(\'" + e.row.checkindetl_id+"\') ><span class='mini-button-text mini-button-icon icon-node'>&nbsp;</span></a>";
	  //      str += "</span>";
	  //      return str;
	  //  }
	    
	 //   function ondEdit(checkindetl_id){
	 //       window.location="EditDetailSheetServlet?checkindetl_id=" + checkindetl_id;
	//	}
