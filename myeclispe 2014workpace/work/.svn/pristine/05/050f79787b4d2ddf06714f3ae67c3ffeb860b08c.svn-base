...
public class PrStatisticsServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String viewSql="";
		if(payMethod.equals("")){
			viewSql="create or replace view prdetailView as select M.* from prdetail M ";//left join warehouse t  on m.warehouseId = t.warehouse_id
			
		}else{
			viewSql="create or replace view prdetailView as select M.* from prdetail M  where paymethod='"+payMethod+"'";//left join warehouse t  on m.warehouseId = t.warehouse_id
		}
