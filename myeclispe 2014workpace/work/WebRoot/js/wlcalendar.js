...
var calendar=function(b)
{
	...
	p.applnk(J.path()+'../css/wllhgcalendar.css');
	...
	if(J.ie){try{document.execCommand('BackgroundImageCache',false,true);
	}catch(e){}};
	};
	J.exend(calendar.prototype,{get:function(obj)
{var ev=J.gtev(),e=ev.srcElement||ev.target,id;if(obj&&obj.id)id=J(obj.id).val();
else id=e.value;
if(obj&&obj.to)
this.to(obj.to);
else
...
this.draw(id);
this.inu=(obj&&obj.id)?obj.id:e;this.type=(obj&&obj.type)?obj.type:'-';
this.time=(obj&&obj.time)?obj.time:false;if(obj&&obj.dir=='right')
...
J(chead).stcs({width:'154px',height:'30px'}).html(txt);
J(m).apch(chead);
...
var cbody=J(d).crte('div');J(cbody).stcs('width','154px').html(txt);J(m).apch(cbody);J('py',d).click(this.py);
J('pm',d).click(this.pm);
J('ny',d).click(this.ny);
J('nm',d).click(this.nm);
J('year',d).click(this.year);
J('month',d).click(this.month);
J('t1',d).hover(function(){J(this).stcs({border:'1px solid #0a246a',backgroundColor:'#c2cbe0'});
},function(){J(this).stcs({border:'1px solid #e3e3e3',backgroundColor:'#f7f7f7'});
}).click(function(){var d=new Date(),y=d.getFullYear(),m=d.getMonth()+ 1,t=d.getDate();
if(J.calendar.type=='-')
...
while(a.length)
{var row=J(doc).crte('tr');for(var i=0;i<7;i++)
{var cell=J(doc).crte('td');J(cell).acls('tdday').html('&nbsp;');if(a.length)
{var day=a.shift();
if(day)
...
{if(this.y>this.ty||(this.y==this.ty&&this.m>this.tm)||(this.y==this.ty&&this.m==this.tm&&day>this.td))
{if(day==this.d)J(cell).acls('tday');J(cell).html(day).msover(this.over).msout(this.out).click(this.setdate);
}
else
J(cell).html(day).stcs('color','#999');
}
...
{if(day==this.d)J(cell).acls('tday');
J(cell).html(day).msover(this.over).msout(this.out).click(this.setdate);}}}J(row).apch(cell);
}J(frag).apch(row);
}
...
J(this._p._ifrm).stcs({height:iw+'px'});
},to:function(l)
...
this.than=a[1];this.ty=d.getFullYear();
this.tm=d.getMonth()+ 1;this.td=d.getDate();
},py:function()
...
if(c.m<10)c.m='0'+ c.m;
if(d<10)d='0'+ d;time=c.time?new Date().toLocaleTimeString():'';if(time!='')time=time.length<8?' 0'+ time:' '+ time;
if(c.type=='-')
J(c.inu).val(c.y+'-'+ c.m+'-'+ d+ time);
else
J(c.inu).val(c.m+'/'+ d+'/'+ c.y+ time);
c._p.hide(this,c._p);
},over:function()
...
J('iy',obj._doc).click(function(e){var ev=e||obj._ifrm.contentWindow.event;ev.stopPropagation?ev.stopPropagation():(ev.cancelBubble=true);
});
},month:function()
...
J('im',obj._doc).click(function(e){var ev=e||obj._ifrm.contentWindow.event;ev.stopPropagation?ev.stopPropagation():(ev.cancelBubble=true);
});
},c_clean:function()
{this.type=null;
this.than=null;
