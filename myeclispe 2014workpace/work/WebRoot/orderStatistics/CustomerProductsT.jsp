...
%>
<%@page import="com.wl.testaction.warehouse.orderstatistics.BeanPara"%>
  	...
  	<form id="form1">
   		...
   		<!-- 表头 -->
   			...
   			<tr style= "height:20px;">
 				...
 				<td>
  					<input value="返回订单" type="button" style= "width:70px;" onclick="back()" />
   	 ...
   	 <div id= "userdiv">
   		...
   		<tr>
            ...
            <input id="rowIndex"  name="row" class="mini-hidden" width="100%"   value="${param.rowIndex}" enabled="false"/>
   			<input id="page"  name="page" class="mini-hidden" width="100%"   value="${param.page}" enabled="false"/>
   			
   			<input id="bdate"  name="bdate" class="mini-hidden"  value="<%=((BeanPara)session.getAttribute("backPara")).getBdate()%>"  />
   			<input id="edate"  name="edate" class="mini-hidden"    value="<%=((BeanPara)session.getAttribute("backPara")).getEdate()%>"  />
   			<input id="customer"  name="customer" class="mini-hidden"   value="<%=((BeanPara)session.getAttribute("backPara")).getCustomer()%>"  />
   			<input id="orderPage"  name="orderPage" class="mini-hidden"  value="<%=((BeanPara)session.getAttribute("backPara")).getPage()%>"   />
   			<input id="orderRow"  name="orderRow" class="mini-hidden"   value="<%=((BeanPara)session.getAttribute("backPara")).getRowIndex()%>"  />
    ...
    <script type="text/javascript">
	   ...
	   search();
	    function search(){
	    	...
	    	grid.load({orderId:orderId,para:page},function(){
				var row=grid.getRow(rowIndex);
				grid.setPageIndex(page);
	    		grid.select(row);
	    		grid.setUrl("LoadCustomerProducts?para=");
			});
	    ...
	    function back(){
	    	...
	    	var bdate = mini.get("bdate").getValue();
	    	var edate = mini.get("edate").getValue();
	    	var customer = mini.get("customer").getValue();
	    	
	    	var page = mini.get("orderPage").getValue();
	    	var row = mini.get("orderRow").getValue();
