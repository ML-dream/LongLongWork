...
<html>
  ...
  <body>
    <body>
		...
		<div id="taskWindow" class="mini-window" title="编辑工序" style="width:280px;height:180px;">
        	<table style="font-size:13px;">
        	    ...
        	    <tr>
                	<td>进度：</td>
                	<td><input id="taskPercentComplete" class="mini-spinner" readonly="readonly"/></td>
            	</tr> 
	...
	<script type="text/javascript">
		...
		//注释：span 没有固定的格式表//。当对它应用样式时，它才会产生视觉上的变化。
		gantt.setColumns([
    		{ 	header: '<span style="font-weight:bold;font-size:13px;color:#44474a;">工序编号</span>', 
        		field: "Name",
         		width: 180,
          		name: "taskname",
           		cellCls: "cellbg",
		...
		//设置时间线
		function setTimes(){
		    url = 'CheckEndTimeServlet';
		    var params = JSON.stringify('${sessionScope.willSeeParts}');
		    params = {'data':params};
		    jQuery.post(url, params, function(text){
 	   			var text = mini.decode(text);
 	   		    gantt.setTimeLines([
			    { 	date: new Date(text.planBTime), text: "时间线" },
			    { 	date: new Date(text.planETime), text: "时间线2", style: "width:2px;background:red;" }
		                       ]);
 	   		},'json');
		}
		setTimes();
//		gantt.setTimeLines([
//			{ 	date: new Date(2016, 10, 1), text: "时间线" },
//			{ 	date: new Date(2016, 9, 1), text: "时间线2", style: "width:2px;background:red;" }
//		]);
      
		function load() {
		    gantt.loading();
		    url = 'PartPlanBaseServlet?meth=seePartsPlan&pathTo=';
...
//		                + "<div style='float:right;'>工期："+task.Duration + "日</div></div>"
		                + "<div style='clear:both;'>开始日期：" + mini.formatDate(task.Start, 'yyyy-MM-dd hh-mm-dd') + "</div>"
		                + "<div>完成日期：" + mini.formatDate(task.Finish, 'yyyy-MM-dd hh-mm-dd') + "</div>"
