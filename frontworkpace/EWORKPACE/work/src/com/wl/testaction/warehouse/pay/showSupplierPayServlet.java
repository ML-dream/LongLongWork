...
public class showSupplierPayServlet extends HttpServlet {

	/**
	 * Constructor of the object.
	 */
	public showSupplierPayServlet() {
		super();
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(startDate.equals("")&&endDate.endsWith("")){
			totalCountSql="select count(*) from supplierpayment where companyid='"+companyId+"'";
			paySql="select paydate,thispaid,qualitykk,tax,memo from(select A.*,rownum row_num from(select EM.* from supplierpayment EM where " +
			...
			totalCountSql="select count(*) from supplierpayment where companyid='"+companyId+"' and paydate between to_date('"+startDate+"','yyyy-mm-dd,hh24:mi:ss') " +
