...
public class NoticeListServlet extends HttpServlet {
	...
	throws ServletException, IOException {
	    ...
	    String totalCountSql = "select count(*) from notice where isReaded=? and receiver=?  ";
	    String[] paramss = {isReaded,createPerson};
		try {
	    ...
	    String noticeWillSql = "select A.id,B.staff_name sender,C.staff_name receiver,sendTime,readTime," +
	    		...
	    		"from (select Z.*,ROWNUM row_num from (select EM.* from notice EM where isReaded=? and receiver=? order by to_number(grade) desc, "+orderStr+" desc) Z where ROWNUM<="+(countPerPage*pageNo)+"  order by  to_number(grade) desc, "+orderStr+"   desc) A " +
	    ...
	    String[] params = {isReaded,createPerson};
