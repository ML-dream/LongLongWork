...
public class ProcessOutAssistStatServlet extends HttpServlet {
...
//    else
//        sql="select distinct to_char(H.deliverTime,'yyyy-mm-dd') deliverTime,A.orderId,A.productId,A.operId,A.issueNum,A.num," +
//        		"to_char(A.planEndTime,'yyyy-mm-dd') planEndTime,A.waiXieCom,B.companyName,B.Connector,B.connectorTel,B.Address," +
//        		"C.product_name productName,E.FO_OPNAME OPERNAME,E.workbranch,F.typeName " +
//        		"from processesplan A " +
//        		"left join outAssistCom B ON A.waixieCom=B.companyId " +
//        		"left join order_detail C on A.orderId=C.order_Id and A.productId=C.product_Id and A.issueNum=C.issue_Num "+
//        		"left join foheader D on D.ORDERID=A.ORDERID AND D.PRODUCTID=A.PRODUCTID AND D.ISSUENUM=A.ISSUENUM "+
...
//        		"left join workbranch F on F.typeId=E.workbranch " +
//        		"left join outAssistmenu H ON H.menuId=A.MENUID "+
//        		"where A.orderId=? and A.productId=? and A.issueNum=? and A.operId=? and A.isdiscard='0' ";
    ...
    request.getRequestDispatcher("outAssistManage/processOutAssistStat.jsp").forward(request,response);
	}
