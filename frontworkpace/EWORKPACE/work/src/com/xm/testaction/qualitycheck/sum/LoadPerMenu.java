...
public class LoadPerMenu extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
	    ...
	    String staffcode =  ((User)session.getAttribute("user")).getStaffCode();	//当前用户
	    String para = request.getParameter("staffcode");	//查询的员工编号
	    String sqla =
	    	"select t.pageid id,t.text ,t.iconcls iconCls,t.pid,t.pagelevel from LISTTREE t " +
	    		"left join rightassign a on a.pageid = t.pageid " +
	    		"where a.staffcode = '"+para+"' "+
	    		"union all " +
	    		"select c.pageid,c.text,c.iconcls,c.pid,c.pagelevel from listtree c where c.pagelevel ='1'" +
	    		"order by id" ;
