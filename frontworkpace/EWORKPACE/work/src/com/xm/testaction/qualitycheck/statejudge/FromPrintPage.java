...
public class FromPrintPage extends HttpServlet {

	/**
	 * Constructor of the object.
	 */
	public FromPrintPage() {
		super();
	}
...
//			bean.setCheckrank(Rs.getString(2));
			String rank = Rs.getString(2);	//审理级别
			if(rank.equals("default")){
				bean.setCheckrank("默认");
			}else if(rank.equals("first")){
				bean.setCheckrank("一级");
			}else if(rank.equals("second")){
				bean.setCheckrank("二级");
			}else{
				bean.setCheckrank("三级");
			}
...
//			bean.setOccurplace(Rs.getString(4));
			...
			if(source.equals("self")){
				bean.setProsource("本厂");
			}else{
				bean.setProsource("外协");
			}
			
			bean.setDuty_part(Rs.getString(42));		//责任部门
			bean.setDuty_man(Rs.getString(41));
			...
			String opinion = Rs.getString(21);
			if(opinion.equals("toDiscard")){
				bean.setOpinion("报废");
			}else if(opinion.equals("toSale")){
				bean.setOpinion("超差使用");
			}else if(opinion.equals("none")){
				bean.setOpinion("无");
			}else {
				bean.setOpinion("返修");
			}
