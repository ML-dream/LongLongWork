  	...
  	<form id="form1">
   		...
   		<!-- 表头 -->
   			...
   			<tr style= "height:20px;">
 				<td>开始时间</td>
				<td>
					<input id="bdate" name ="bdate" class="mini-datepicker" dateFormat="yyyy-MM-dd" allowInput="true" value="${param.bdate}" />
 				</td>
 				<td>结束时间</td>
				<td>
					<input id="edate" name ="edate" class="mini-datepicker" dateFormat="yyyy-MM-dd" allowInput="true" value="${param.edate}"/>
 				</td>
 				<td>
 				...
 				<input id="rowIndex"  name="rowIndex" class="mini-hidden" width="100%"   value="${param.row}" enabled="false"/>
    ...
    <script type="text/javascript">
	   ...
	   function onDrawSummaryCell(e) {
	    ...
	    function search(){
	    	...
	    	grid.load({bdate:bdate, edate:edate,customer:customer,para:page},function(){
				var row=grid.getRow(rowIndex);
				grid.setPageIndex(page);
	    		grid.select(row);
	    		grid.setUrl("LoadCustomerOrders?para=");
			});
	    ...
	    function seePay(){
	    	...
	    	window.location="getCompanyIdServlet?companyId="+customer;
	    }
