...
<html>
  <head>
	...
	<body>
    	<form id="form_cmtyf" name="form_cmtyf" action="" method=post>
    		<table class="green_list_table" align="center" width="100%" height="50%" border="1" style="word-break:break-all;border-collapse:collapse" bgcolor="#dafec5">
    			<tr>
      				<th>工序选择</th>
      				<td><!--<select id="oper_id" name="oper_id" style="width:130px;">
	          				<option value="">---暂无数据---</option>
	         			</select>-->
	         			<input id="foNo"  name="foNo" class="mini-textbox"  width="100%" required="true" readonly value="${foNo}"/></td>
	         	</tr>
   				<tr><th>使用刀具</th>
				...
				<tr><td><a class="mini-button" iconCls="icon-save" plain="false"  onclick="toservlet()">保存</a>
	     			...
	     			<input id="productId"  name="productId" class="mini-hidden"  width="100%" value="${productId}"/>
	     			<input id="issueNum"  name="issueNum" class="mini-hidden"  width="100%" value="${issueNum}"/>
	     			<input id="foId" name="foId" class="mini-hidden"  width="100%" value="${foId}"/>
				</tr>
			</table>
			<br>
		</form>
	...
	<div id="grid1" class="mini-datagrid" style="width:100%;height:300px;"
         	...
         	url="BaseServlet?meth=CMTYFList&productId=${productId}&issueNum=${issueNum}">
        <div property="columns">
            <div type="indexcolumn"></div>
            <div name="action" width="40" headerAlign="center" align="center" renderer="onOperatePower"cellStyle="padding:0;">操作</div>
            <div field="foNo" width="60" headerAlign="center">工序编号</div>
	...
	<script>
        ...
        mini.get("issueNum").value = Request["issueNum"];
   		...
   		function onOperatePower(e) {
			var str = "";
			str += "<span>";
			str += "<a style='margin-right:2px;' title='编辑' href=javascript:ondEdit(\'" + e.row.productId+"\',\'" + e.row.issueNum+"\',\'" + e.row.foNo+"\',\'" + e.row.foId+"\') ><span class='mini-button-text mini-button-icon icon-edit'>&nbsp;&nbsp;</span></a>";
			str += "</span>";
			return str;
		};
		    
		function ondEdit(productId,issueNum,foNo,foId){
		    window.location.href = "BaseServlet?meth=focmtyfEdit&pathTo=focmtyfEdit&productId="+productId+"&issueNum="+issueNum+"&foNo="+foNo+"&foId="+foId;
		}
   		...
   		function toservlet(){
            ...
            if (form.isValid() == false) {
                ...
                var url = 'BaseServlet?meth=AddCMTYF';
