...
<html>
  ...
  <body>
	...
	<!--<a class="mini-button" plain="false" iconCls="icon-reload" onclick="test()">测试</a>
	--></div>
    ...
    multiSelect="true" idField="id" showSummaryRow="true" allowAlternating="true" showPager="true"
     showFilterRow="true" url="ShowPrServlet">
      	<div property="columns">
    		...
    		<div field="customerName" headerAlign="center" align="center">供应商
    			<input id="customerFilter" name="customerFilter" property="filter" class="mini-buttonedit" onbuttonclick="onSupplierButtonEdit" textName="customerName"
    			 allowInput="false" style="width:100%;" onvaluechanged="onFilterChanged" showClose="true" onCloseClick="onCustomerClose('customerFilter')"/></div>
...
<!--    		<div field="examineName" headerAlign="center" align="center">审核人</div>-->
    ...
    grid.load({ sheetid: "",date:"",customerId:"",isbill:""});
    
    
    function test (){
    	//grid.reload();
    	
		grid.load({ sheetid: "",date:"",warehouseId:"",customerId:"",isbill:"" },function(){
    ...
    var str="";
    str+="<span>";
    str+="<a style='margin-right:2px;' title='修改' href=javascript:ondSee(\'"+e.row.prSheetid+"\')><span class='mini-button-text mini-button-icon icon-edit'>&nbsp;</span></a>";
    str+="</span>";
    str+="<span>";
    ...
    function ondSee(prSheetid){
    window.location="seePrDetailServlet?prSheetid="+prSheetid;		// 修改 
    
    }
    
    function ondEdit(prSheetid){
    window.location="editPrServlet?prSheetid="+prSheetid;		//查看 
    ...
    function onFilterChanged() {
            ...
            grid.load({ sheetid: sheetid,date:date,customerId:customerId,isbill:isbill });            

        }
        
    function onCustomerClose(customerFilter){
    	mini.get(customerFilter).setValue("");
    	mini.get(customerFilter).setText("");	
    	var datebox = mini.get("dateFilter");
        var sheetidbox = mini.get("sheetidFilter");
		var customerbox=mini.get("customerFilter");
		var billbox=mini.get("billFilter");
