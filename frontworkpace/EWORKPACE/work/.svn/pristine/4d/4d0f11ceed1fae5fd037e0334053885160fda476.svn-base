	...
	<body style="height: 100%;">
		<div id="layout1" class="mini-layout" style="width:100%;height:100%;">
		    <div showHeader="false" region="west" width="180" maxWidth="250" minWidth="100" >
		        ...
		        <ul id="leftTree" class="mini-tree" style="width:100%;height:95%;"
                    ...
                    showCheckBox="true" checkRecursive="false" showFolderCheckBox="true"
		...
		<script>
	        ...
	        function onNodeSelect(e) {
	        	...
	        	if(node.level==2){			//零件层
	        		iframe.src = "DiscardBaseServlet?meth=GoMainBody&pathTo=discardMainBody&orderId="+node.orderId+"&productId="+node.id+"&FProductId="+node.pid+"&issueNum="+node.issueNum+"&drawingId="+node.drawingId+"&barcode="+node.barcode;
	        		//mini.alert("请选择具体订单项！");
		        }
        	...
        	function listDetail(e){
    			...
    			for(var i=0,len=nodes.length;i<len;i++){
    				...
    				var subdata = "{'orderId':'"+orderId+"','productId':'"+productId+"','issueNum':'"+issueNum+"','productName':'"+productName+"','drawingId':'"+drawingId+"','barCode':'"+barCode+"'}";
    				data[j] = subdata;
    				j++;
    			}
    			...
    			params = {'data':data};
    			var url = 'DiscardBaseServlet?meth=GoSeePlanDetail&pathTo=';
    			jQuery.post(url, params, function(data){
 	   					iframe.src = '<%=path %>/partPlanManage/SeeDiscardPlanDetail.jsp';
 	   				});    			
        	}
...
//            			}
    			...
    			for(var i=0,len=nodes.length;i<len;i++){
    				...
    				var subdata = "{'orderId':'"+orderId+"','productId':'"+productId+"','issueNum':'"+issueNum+"','productName':'"+productName+"','sortie':'"+sortie+"','drawingId':'"+drawingId+"','barcode':'"+barcode+"'}";
    				data[j] = subdata;
    				//alert(data[j]);
    				j++;
    			}
    			...
    			var url = 'DiscardBaseServlet?meth=AddpartsPlan&pathTo=';
    			jQuery.post(url, params, function(data){
 	   				tree.load();
 	   				/*
        	...
        	function seePartsPlan(){
    			...
    			var pageTO = '';
    			if(nodes.length==1){
    				pageTO = 'discardPartProGT.jsp';
    			}else{
    				pageTO = 'discardPartGT.jsp';
    			}
    			
    			var url = 'DiscardBaseServlet?meth=GoPartsPlanGandT&pathTo=';
    			jQuery.post(url, params, function(data){
