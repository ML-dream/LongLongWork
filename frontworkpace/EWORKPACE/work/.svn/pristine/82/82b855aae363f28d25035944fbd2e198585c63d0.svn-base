...
.mini-rgantt .myitem2 .mini-gantt-percentcomplete {
	...
	<body>
		...
		<input type="button" id="linked" value="建立任务关联" onclick="linked()" />
		<br />
		...
		<input type="button" onclick="add_part();" value="添加零件"/>
		<input type="button" onclick="window.location.href='/test/partplan_oper.do?flag=query_partplan';" value="添加工序"/>
...
function load() {
    ...
    $.ajax({
       ...
       url: "../JSP/testTrans.jsp?d=" + new Date().getTime(),
       //url: "/test/showGT.do?flag=load",
