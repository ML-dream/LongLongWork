...
public class LoadWaitFix extends HttpServlet {
//  加载未处理 返修卡
	/**
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String sql="select * from "+
			"(select z.*,rownum rn from(select a.runnum,c.fo_no,c.occurdate,c.barcode,b.fo_opname,b.checkdate from fixdetail a "+
	        "left join reject_state c on c.runnum = a.staterunnum "+
			"left join po_routing2 b on b.barcode =c.barcode and b.fo_no = c.fo_no "+
	       "where a.ishandle=0 order by runnum desc)z )"+
	       "where rn>"+min+" and rn <= "+max;
		
		String totalsql="select count(*) from (select runnum from fixdetail where ishandle=0)";
		int total = 0;
