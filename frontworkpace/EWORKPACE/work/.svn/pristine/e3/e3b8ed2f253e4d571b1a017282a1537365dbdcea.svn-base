...
//	and orderid like '"+orderId+"%'   and orderid like '"+orderId+"%'
		...
		String totalCountSql="select count(*) from po_plan where status>=2 and status<=4 and po_sheetid like '"+poSheetid+"%' and to_char(postart_date,'yyyy-MM-dd hh24:mi:ss') like '"+postartDate+"%' " +
		...
		String poSql="select POSTART_DATE,PO_SHEETID,DRAWERID,C.STAFF_NAME drawerName,STATUS,EXAMINEID,E.staff_name examinename,orderid,CUSTOMERID,D.companyname customername " +
				...
				" and status like '"+status+"%' and drawerid like '"+drawerId+"%' and examineid like '"+examineId+"%' and customerid like '"+companyId+"%' order by po_sheetid desc) A where rownum<="+(pageSize*pageNow)+"" +
						" po_sheetid desc) B " +
