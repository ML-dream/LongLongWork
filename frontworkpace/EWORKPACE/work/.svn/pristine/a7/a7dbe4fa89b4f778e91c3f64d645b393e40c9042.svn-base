...
<html>
  ...
  <body>
	    ...
	    <table >
	       ...
	       <tr>
	          ...
	          <td><input id="status" name="status" class="mini-combobox" width="100" textName="" textField="text" valueField="id"
  				url="data/orderConfirm.txt"  allowInput="false" showNullItem="true" nullItemText="请选择..."  onvaluechanged="loadgrid"/>
	          </td>
    ...
    <div id="grid1" class="mini-datagrid" style="width:100%;height:95%;"
         ...
         url="OrderConfirmList">
        <div property="columns">
            <div type="indexcolumn"></div>
            <div name="action" width="40" headerAlign="center" align="center" renderer="onOperatePower"
                 cellStyle="padding:0;">操作
            </div>
            <div field="orderId" width="120" headerAlign="center">订单编号
            </div>
            <div field="companyName" width="100" headerAlign="center">客户名称
            </div>
            <div field="connector" width="60" headerAlign="center">联系人
            </div>
            <div field="connectorTel" width="100" headerAlign="center">联系人电话
            </div>
            <div field="deptName" width="60" headerAlign="center">使用部门
            </div>
            <div field="orderDate" width="100" headerAlign="center" dateFormat="yyyy 年 MM 月 dd 日">订单日期
            </div>
    ...
    <script type="text/javascript">
	    ...
	    function onOperatePower(e) {
	        var str = "";
	        str += "<span>";
	        str += "<a style='margin-right:2px;' title='订单详情' href=javascript:onDetail(\'" + e.row.orderId+"\') ><span class='mini-button-text mini-button-icon icon-node'>&nbsp;</span></a>";
	        str += "</span>";
	        /*
	        str += "<span>";
		...
		function onDetail(orderId){
			//window.location="GoCheckOrderDetailListServlet?orderId="+orderId;
			//window.location="orderManage/QuotationMainIndex.jsp?orderId=" + orderId+"&connector="+connector;
			window.location.href="GoOrderConfirmServlet?orderId=" + orderId;
		}
		...
		function pass(orderStatus){
            ...
            if (row) {
                mini.open({
                    url: "orderManage/OrderConfirmAdvice.jsp?orderId="+row.orderId+"&orderStatus="+orderStatus,
                    title: "审核意见", width: 600, height: 400,
                    onload: function () {
                        var iframe = this.getIFrameEl();
                        var data = { action: "edit", orderId: row.orderId };
