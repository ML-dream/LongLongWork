...
<html>
  ...
  <body>
  	<div class="mini-toolbar">
	    ...
	    <span class="separator"></span>
  		<a class="mini-button" plain="false" iconCls="icon-goto" onclick="reset()">重置密码</a>
  	</div>
   	...
   	<div id= "userdiv">
       	...
       	<tr>
            ...
            <td><input id="authority" name="authority" class="mini-combobox" style="width:100%;" textField="authorityName" valueField="authorityId" emptyText="请选择..."
    			url="GetAuthorityServlet" required="true" allowInput="false" showNullItem="true" nullItemText="请选择..." enabled="false" value="${result.authority}"/>  
            </td>
   ...
   <script>
   		mini.parse();
   		function reset(){
   			mini.confirm("当前用户密码将被重置为123", "确定？",
                function (action){            
                  if (action == "ok") {
                  	var form = new mini.Form("#userdiv");
                 	var data = form.getData();
	                var params = eval("("+mini.encode(data)+")");
	                var url = 'DoEditSysUsersServlet?para=1';		//参数1 表示重置密码
