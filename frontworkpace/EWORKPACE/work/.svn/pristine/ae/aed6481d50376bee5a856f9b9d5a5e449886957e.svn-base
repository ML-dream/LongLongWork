...
public class examineProductCheckinServlet extends HttpServlet {

	/**
	 * Constructor of the object.
	 */
	public examineProductCheckinServlet() {
		super();
	}
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if(startDate.equals("")||endDate.equals("")){
			...
			"from (select A.*,rownum row_num from (select EM.* from productcheckin EM where status=0 order by checkindate desc,checksheetid desc ) A where rownum<="+(pageSize*pageNow)+" " +
			...
			checkinSql="select checksheetid,checkindate,orderid,productid,productname,drawingid,spec,B.createperson,C.user_name createpersonName,status " +
			"from (select A.*,rownum row_num from (select EM.* from productcheckin EM where status=0 and checkindate between to_date('"+startDate+"','yyyy-mm-dd,hh24:mi:ss') and " +
			"to_date('"+endDate+"','yyyy-mm-dd,hh24:mi:ss')  order by checkindate desc,checksheetid desc ) A where rownum<="+(pageSize*pageNow)+" " +
			"order by checkindate desc,checksheetid desc ) B " +
			"left join sysusers C on C.staff_code=B.createperson " +
			"where row_num>="+(pageSize*(pageNow-1)+1)+" order by checkindate desc,checksheetid desc";
		}
